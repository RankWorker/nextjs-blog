...
export async function generateProject(options: InitOptions): Promise<GeneratedProject> {
  ...
  if (options.source === "local-mdx") {
    const contentRoot = path.resolve(
      options.cwd,
      options.contentDirectory ?? "content/blog",
    );
    files.set(
      path.join(contentRoot, "welcome-to-your-blog.mdx"),
      exampleArticleTemplate(),
    );
  }
...
function exampleArticleTemplate(): string {
  return `---
title: "Welcome to your blog"
description: "A sample MDX article generated by the RankWorker blog initializer."
excerpt: "Use this article as a starting point for your first locally managed post."
slug: "welcome-to-your-blog"
date: "2026-01-01"
cover: "https://picsum.photos/seed/rankworker-blog/1600/900"
coverAlt: "Placeholder cover for the welcome article"
keywords:
  - "example"
  - "mdx"
tags:
...
Replace this sample with your own article, or keep it nearby as a reference for the required frontmatter fields.
`;
}
...
async function wrapNextConfig(
  ...
  if (isCommonJs) {
    ...
    if (new RegExp(`^${helper}\\s*\\(`).test(expression)) {
  ...
  const defaultExport = findDefaultExportExpression(source);
  if (!defaultExport) return { updated: false, alreadyWrapped: false };
  if (new RegExp(`^${helper}\\s*\\(`).test(defaultExport.expression)) {
    return { updated: false, alreadyWrapped: true };
  }

  const content = `${source.slice(0, defaultExport.index)}export default ${helper}(${defaultExport.expression});\n`;
  await writeFile(
    configPath,
...
function findDefaultExportExpression(
  source: string,
): { index: number; expression: string } | undefined {
  const withSemicolon = /export\s+default\s+([\s\S]*?)\s*;\s*$/.exec(source);
  const withoutSemicolon = /export\s+default\s+([\s\S]*\S)\s*$/.exec(source);
  const match = withSemicolon ?? withoutSemicolon;
  const expression = match?.[1]?.trim();
  if (!match || !expression) return undefined;

  // Declarations cannot be wrapped as expressions without rewriting their syntax.
  if (/^(?:async\s+)?function\b|^class\b/.test(expression)) return undefined;
  return { index: match.index, expression };
}
